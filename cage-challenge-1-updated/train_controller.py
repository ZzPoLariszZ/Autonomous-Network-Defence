...
if __name__ == "__main__":
    print("Training CTRL agent...", flush=True)
    gpus = 0
    port = 6379
    if len(sys.argv) > 1:
        gpus = int(sys.argv[1])
        port = sys.argv[2]
        print("Ray config:   GPUs:", str(gpus), " |  Port:", port)
    ...
    config = Trainer.merge_trainer_configs(
        ...
        "vf_loss_coeff": 0.005,  # Scales down the value function loss for better comvergence with PPO
    })
