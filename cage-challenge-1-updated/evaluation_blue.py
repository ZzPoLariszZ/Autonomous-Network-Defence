...
from CybORG.Agents import B_lineAgent, CovertAgent, RedMeanderAgent, RandomAgent, SleepAgent
...
from agents.hierachy_agents.load_blue_agent import LoadBlueAgent
...
randomagents = [B_lineAgent, CovertAgent, RedMeanderAgent, RandomAgent]
...
if __name__ == "__main__":
    print("Running mixnet simulator...", flush=True)
    gpus = 0
    port = 6379
    if len(sys.argv) > 1:
        gpus = sys.argv[1]
        ...
        print("No params provided. Using default Ray config.")
    
    print("Ray config:   GPUs:", gpus, " |  Port:", port)
    ##############################################################
    ...
    for num_steps in [100]:
        ...
        for i in range(MAX_EPS):
            ...
            #for red_agent in :
            red_agent = np.random.choice(randomagents, p=[0.25, 0.25, 0.25, 0.25])
            # red_agent = np.random.choice(randomagents, p=[0, 0, 0, 0, 1])
            ...
            for j in range(num_steps):
                ...
                a.append((str(cyborg.get_last_action('Blue')), str(cyborg.get_last_action('Green')), str(cyborg.get_last_action('Red'))))
