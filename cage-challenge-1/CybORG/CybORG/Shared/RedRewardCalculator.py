...
from CybORG.Shared.Actions import FindFlag, Impact, DefenceEvade
                ...
                for session in info['Sessions']:
                    ...
                    if session['Agent'] == self.agent_name:
                        ...
                        if session['Username'] == 'SYSTEM' and info['System info']['OSType'] == OperatingSystemType.WINDOWS:
                            ...
                            break
                        # count the number of Hidden sessions
                        if session['Username'] == 'Hidden':
                            hidden_sessions += self.mapping[self.scenario.get_host(host).get('ConfidentialityValue', 'Low')]
                            break

        # # instant reward for hidding
        hide_reward = 0
        agent_action = action[self.agent_name]
        if type(agent_action) is DefenceEvade and agent_observations[self.agent_name].data['success'] == True:
            hide_base = self.mapping[self.scenario.get_host(agent_action.hostname).get('AvailabilityValue', 'Low')] 
            if hide_base == 0.0:
                hide_base = 0.1
            hide_reward = hide_base * HIDE_SCALED

        # find the difference from the old privileged sessions
        total = root_sessions + system_sessions + hidden_sessions + hide_reward
...
class HybridImpactPwnRewardCalculator(RewardCalculator):
    ...
    def calculate_reward(self, current_state: dict, action: dict, agent_observations: dict, done: bool) -> float:
        ...
        reward = pwn_reward + disrupt_reward
