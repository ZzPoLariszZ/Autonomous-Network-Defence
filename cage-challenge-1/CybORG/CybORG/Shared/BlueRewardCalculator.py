...
class HybridAvailabilityConfidentialityRewardCalculator(RewardCalculator):
    ...
    def calculate_reward(self, current_state: dict, action: dict, agent_observations: dict, done: bool) -> float:
        ...
        availability_reward = self.availability_calculator.calculate_reward(current_state, action, agent_observations, done)
        # print('Blue - availability_reward: ' + str(availability_reward))
        confidentiality_reward = self.confidentiality_calculator.calculate_reward(current_state, action, agent_observations, done)
        # print('Blue - confidentiality_reward: ' + str(confidentiality_reward))
        # print('-' * 76)

        reward = availability_reward + confidentiality_reward
